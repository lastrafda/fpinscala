...
object MyProgram:
  ...
  def fib(n: Int): Int =
    @annotation.tailrec
    def go(n: Int, prev: Int, beforePrev: Int): Int =
      if (n <= 0) then beforePrev
      // else if (n == 2) then prev + beforePrev
      else go(n - 1, prev + beforePrev, prev)
    go(n, 1, 0)

  // Paul's solution is simpler, in both cases it seems like
  // def fib(n: Int): Int =
  //    @annotation.tailrec
...
object PolymorphicFunctions:
  ...
  // Paul's solution is soo much better, I couldn't see it
  // def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean =
  //   @annotation.tailrec
